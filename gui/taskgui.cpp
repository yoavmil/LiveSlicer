...
void TaskGui::connectToTask(Task *_task)
{
    DBGF;
    task = _task;
    connect(task, SIGNAL(progress(int)), this, SLOT(progress(int)), Qt::QueuedConnection);
    connect(task, SIGNAL(finished(bool)), this, SLOT(finished(bool)), Qt::QueuedConnection);
