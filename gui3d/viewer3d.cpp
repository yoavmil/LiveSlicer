    ...
    provider(nullptr)
{
    ...
    trackBall = new TrackBall(0.0f, QVector3D(0, 0, 1), TrackBall::Plane);
}

#define PIXEL_TO_VIEW_POS(_p) QPointF(2.0*float(_p.x())/width()-1.0, 1.0-2.0*float(_p.y())/height())
...
void Viewer3D::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::LeftButton) {
        trackBall->push(PIXEL_TO_VIEW_POS(event->pos()));
        event->accept();
    }
    update();
}
...
void Viewer3D::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::RightButton) {
        trackBall->release(PIXEL_TO_VIEW_POS(event->pos()), QQuaternion());
        event->accept();
    }
    update();
}
...
void Viewer3D::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::RightButton) {
        trackBall->move(PIXEL_TO_VIEW_POS(event->pos()), QQuaternion());
        event->accept();
    }
    update();
}
